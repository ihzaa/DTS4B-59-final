                ...
                <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                        <Stack direction="row" spacing={1}>
                            {movie.genres && movie.genres.map(genre => <Chip key={genre.id} label={genre.name} color={colors[Math.floor(Math.random() * 3)]} />)}
                        </Stack>
                    </Typography>
                    <Divider sx={{ my: 1 }} />
                    <Typography gutterBottom variant="h6" component="div">
                    ...
                    <Typography variant="body2" color="text.secondary">
                        <Rating name="half-rating-read" defaultValue={movie.vote_average && movie.vote_average.toFixed(1)} precision={0.1} max={10} readOnly sx={{ mr: 1 }} />
                        ({movie.vote_average && movie.vote_average.toFixed(1)}) from {movie.vote_count} user{movie.vote_count > 1 ? 's' : ''}.
                    </Typography>
                    <Divider sx={{ my: 1 }} />
                    <Typography gutterBottom variant="h6" component="div">
                        Popularity
                    ...
                    <Typography variant="body2" color="text.secondary">
                        <Stack direction="row" spacing={1}>
                            {movie.production_companies && movie.production_companies.map(production_company =>
                                <Chip
                                    avatar={<Avatar alt={production_company.name} src={'https://image.tmdb.org/t/p/original' + production_company.logo_path} />}
